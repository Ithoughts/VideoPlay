...
- (void) prepare {
    ...
    // For each target, create a VideoPlayerHelper object and zero the target dimensions
    ...
    // Start video playback from the current position (the beginning) on the first run of the app
    for (int i = 0; i < NUM_VIDEO_TARGETS; ++i) {
...
- (void)createFramebuffer
{
    if (context) {
        ...
        // 请求 OpenGL ES 为图形处理器控制的缓存生成一个独一无二的标志符
